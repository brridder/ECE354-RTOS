...
\begin{table}
    ...
    \hline
    \hline
    \texttt{MESSAGE\_CMD\_REG} & KCD Process & \\
    \texttt{MESSAGE\_KEY\_INPUT} & KCD Process & \\
    \texttt{MESSAGE\_OUTPUT} & CRT Display Process & \\ 
    \texttt{MESSAGE\_OUTPUT\_NO\_NEWLINE} & CRT Display Process & \\
    \texttt{MESSAGE\_COUNT\_REPORT} & User Process C & \\
    \texttt{MESSAGE\_WAKE\_UP\_10} & User Process C & \\
    \hline
    \end{tabular}
    } % Center
...
void* request_memory_block() {
    ...
    if running process is not an i-process:
        if there are no unreserved memory blocks free:
            ...
            enqueue running process to blocked memory priority queue
            call release processor
    else:
...
int release_memory_block(void* memory_block) {
    ...
    if memory block is not allocated or out of range:
        ...
        if there are processes blocked on memory:
            ...
            enqueue unblocked process on ready queue
...
int send_message(int pid, void* message_envelope){
    ...
    if receiving process is blocked on messages:
        ...
        enqueue receiving process to ready queue
