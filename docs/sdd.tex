...
At a high level, the operating system consists of two main parts:
initialization, and running processes. When executing begins, the
operating system initializes memory, processes, priority queues, and
interrupts. Once the initialization is complete, the operating system
is in a state where it can run processes and respond to system calls
as well as hardware interrupts. System call services are provided to
processes running within the operating system in the form of a
software interrupt handler. After initialization, a process is
scheduled and then run. Whenever a system call occurs or a hardware
